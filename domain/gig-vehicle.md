...
## Endorsed heuristics (Blake 2026-09-14, expanded)
...
- **Softened TCO / insurance / SOH language (mandatory):**
  - No hard cutoff like "SOH < 80% → reject."
  - No invented "10–15% insurance" (or similar) as fact.
  - No "guaranteed."
...
- High annual miles for multi-app gigs — use **~25–30k mi/yr** for TCO sketches (often 20k–40k+); when he says **~40k**, default Corolla unless Prius SOH verified. City stop-go, Texas heat, parking-lot loops.
...
- Mixed week: one vehicle rarely dominates both — van for bulk days + cheap car for food, or accept van fuel penalty on food days (qualitative: often meaningful $/year — not fake gallon math).
...
Assume **~25–30k gig miles/year** (multi-app), or **~40k** when he says so — DFW gas/insurance — **order of magnitude only**:

1. **Fuel** — hybrids often win hard on stop-go when the pack is healthy; estimate = miles ÷ mpg × local $/gal **only when doing live math from stated inputs** — otherwise keep qualitative. Fuel savings can be meaningful; battery risk can erase them at high annual miles.
2. **Insurance** — must be a real Fort Worth quote on year/trim; compact sedan usually costs less than van/truck. Never invent percentages like "10–15%."
3. **Maintenance** — Toyota/Honda generally lower; hybrids add battery risk at high miles. Require PPI / SOH report — do not invent hard SOH cutoffs.
4. **Tires** — **one bucket only**; do not also bury tires inside maintenance (no double-count).

Add: registration, unexpected repairs, opportunity cost of downtime. Prefer "rough annual total with uncertainty" over fake precision. Illustrative ranges OK **only if clearly labeled estimate**.
...
- **Gen 2 (~2004–2009):** mpg king when healthy; many high-mile packs are replaced or near EOL — budget a pack or demand proof of recent battery work + SOH report. Never invent a hard SOH % reject line.
- **Gen 3 (~2010–2015):** endorsed best pick **only with verified healthy battery**; at ~40k mi/yr Corolla is safe default if SOH unknown. Neglected examples can show head-gasket / EGR / heat-related issues — PPI matters.
...
- Stop-go favors hybrids and light cars; heavy vans burn more fuel on food-only days (qualitative fuel penalty — meaningful $/year possible; no fake gallon totals).
- Insurance and registration: Texas / Tarrant or nearby — always quote locally. Prefer **Alliance / Fort Worth 76177** — never invent ZIP bands (e.g. 76102–76140).

## 7. Cargo van vs sub-$10k car (his mix)

| Job mix | Prefer | Why |
|---------|--------|-----|
| Mostly DoorDash / Uber Eats / Shipt | Sub-$10k Corolla / verified-battery Prius | MPG + insurance; bag fits; van on food days is a fuel penalty |
| Mostly Roadie / Amazon Flex / bulk | Cargo van he already maintains | Cube + payload |
| Mixed week | Van for bulk days + cheap car for food | Or accept van **fuel penalty** on food days (qualitative: often hundreds of gallons / meaningful $/year — not exact invented arithmetic) |

He **maintains a cargo van** — do not pretend he is starting from zero. Evaluating a **second** cheap car for food apps is rational; sub-$10k car is still worth it for food apps even if the van stays for bulk.
